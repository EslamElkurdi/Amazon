...
export const cartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      state.productsNumber = action.payload.quantity // from 8
      //  state.productsNumber + parseInt(action.payload.quantity);
        
    },
    addQuantityToCart: (state, action) => {// from 8
      console.log(state,action)
      // check if in product array
      const addProductExists = state.products.find(
        (product) => product.id === action.payload.id
...
export const { addToCart,addQuantityToCart, removeFromCart, incrementInCart, decrementInCart } =//from 8
  cartSlice.actions;
